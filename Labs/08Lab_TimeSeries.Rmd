...
Rainfall_Time <- ggplot(BR_complete, aes(x = Date, y = Rainfall))+
  geom_line(color = "blue", size = 0.2)+
  geom_smooth(method = "lm", color = "red")
print(Rainfall_Time)

Temperature_Time <- ggplot(BR_complete, aes(x = Date, y = Temperature_C))+
...
ts_temperature <- ts(BR_complete$Temperature_C, frequency = 12, start=c(f_year, 
                                                                        f_month
                                                                        ))
ts_rainfall <- ts(BR_complete$Rainfall, frequency = 12, start=c(f_year, f_month
                                                                        ))
```
...
stl_temp = stl(ts_temperature, s.window = "periodic")
plot(stl_temp)

stl_rain = stl(ts_rainfall, s.window = "periodic")
plot(stl_rain)
