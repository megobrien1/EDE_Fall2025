...
NTL.phys.data.tues.90.99 <- NTL.phys.data %>%
...
> Answer: Because the rows are not listed in specific order. All of the dates and lake names are listed in random order. You can only filter specific categories within the data by specifying the column names. 
...
NTL.phys.data.tues.90.99.July <- NTL.phys.data %>%
  filter(lakename == 'Tuesday Lake',
         year4 >= 1990,
         year4 <= 1999, 
         month(sampledate) == 7)
...
NTL.phys.data.tues.90.99.July.surface <- NTL.phys.data.tues.90.99.July %>%
    filter(depth == 0)%>%
...
temperatureC <- NTL.phys.data %>%
  pivot_longer(
    cols = c(temperature_C, depth), 
