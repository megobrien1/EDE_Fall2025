...
Temp_day_plot <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
  ggplot(
    mapping = aes(
      x=daynum,
      y=temperature_C, color=year4)
    ) + 
  geom_point() +
...
Airqual_plot <- EPAair %>% 
  ggplot(
    mapping = aes(
      x=PM2.5,
      y=Ozone, color=mean_Lat)
    ) + 
  geom_point(alpha=0.5) +
  labs(
    title="AQI Values for Ozone by PM2.5",
    x = "PM2.5",
    y = "Ozone",
    color = "Latitude")+
