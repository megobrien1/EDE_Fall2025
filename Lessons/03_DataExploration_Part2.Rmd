...
>Answer: You can information on types of data in the dataset and also gain an idea of what kind of range of data you are looking at. This can help when making decisions on how to best analyze the data for what you are hoping to gain insight on. 
...
# Make sure datetime is proper POSIXct
USGS.flow.data$datetime <- as.POSIXct(USGS.flow.data$datetime, tz = "UTC")

# Make sure discharge.mean is numeric
USGS.flow.data$discharge.mean <- as.numeric(USGS.flow.data$discharge.mean)

# Drop rows with NA, NaN, or Inf in either column
USGS.flow.data <- subset(
  USGS.flow.data,
  is.finite(datetime) & is.finite(discharge.mean)
)
...
> Answer: Different data types are more suitably visualized in some graph types more than others. It depends on the kind of data you are looking at and what you want to show. 

## Ending discussion
What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: There is a positive correlation between gage and discharge, and there is a higher count of lower gage height and discharge levels. Different graph types display this data differently. 


How can multiple options for data exploration inform our understanding of our data?

> Answer: Different forms of data exploration can highlight different aspects of the data. This is important to gain a more full understanding of the dataset. 
...
> Answer: The average gage height and average discharge had a positive correlation. This makes sense because as the level of water in the river rose, so did the discharge rate. 
