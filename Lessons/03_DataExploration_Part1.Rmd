...
> Answer: Sometimes a successful package loading does not show anything in the console. It may also show a confirmation message that the package was loaded successfully. 
...
#R will still read it in, but it won't be stored in an object you can use later. 
...
> ANSWER: This file is found in the metadata folder RMD file. The data were collected from the U.S. Geological Survey's Water Data website.
...
> Answer: Removing NAs would be favorable for scientific montioring data where accuracy is essential. Occasinal missing values are expected in this kind of data, and so emitting them could help make the data analysis more reliable.  
...
#USGS.flow.data.time <- USGS.flow.data
#USGS.flow.data.time$datetime <- as.Date(USGS.flow.data$datetime, format = '%Y/%m/%d') 
```
