...
> Add notes here: These can all be indicators of the overall health of lakes. 
...
NTL.phys.data.tues.90.99 <- NTL.phys.data[NTL.phys.data$lakename == "Tuesday Lake" & 
NTL.phys.data$year4 >= 1990 & 
NTL.phys.data$year4 <= 1999, 
]

```
Question: Why don't we filter using row numbers? 

> Answer: The categories are sorted by columns. In order to filter these categories, you need to filter the columns. 
...
When we wrangle a raw dataset into a processed dataset, we create a code file that contains only the wrangling code. We then save the processed dataset as a new spreadsheet and then create a separate code file to analyze and visualize the dataset. Why do we keep the wrangling code separate from the analysis code? # It is important to keep the original data seperate from the processed data in case you want to go back to the original data. 
